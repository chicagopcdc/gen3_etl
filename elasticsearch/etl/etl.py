...
# AWS region for SigV4 request signing. Set to e.g. 'us-east-1' when ES_HOST is an AWS
# OpenSearch endpoint. Leave empty to skip signing (self-managed / local Elasticsearch).
es_aws_region: str = os.environ.get('ES_AWS_REGION', '')
...
def load_data(data: dict[str, any]) -> None:
    ...
    load_es_data(data, es_port, index_name, es_host, es_bulk_batch_size, es_bulk_max_tries, es_bulk_retry_delay, es_timeout, es_scheme, es_aws_region)


def load_array_config() -> None:
    """ Load gen3 data portal Elasticsearch array config index with array fields """
    load_es_array_config(es_port, index_name, es_host, es_scheme, es_aws_region)
...
if len(sys.argv) > 1:
    if sys.argv[1] == 'e':
        ...
        if len(sys.argv) == 5 and sys.argv[2] and sys.argv[3] and sys.argv[4]:
            ...
            switch_alias(es_port, alias, old_index, new_index, es_host, es_scheme, es_aws_region)
        else:
