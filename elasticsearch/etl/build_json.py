...
number_key = ["age_at", "age_at_vitals", "vitals_result_numeric", "age_at_total_dose_start", "age_at_total_dose_end", "total_dose_administered", "age_at_ae", "age_at_medication_start", "age_at_medication_end", "age_at_gts", "age_at_ihc", "ihc_result_numeric", "age_at_rt_start", "age_at_rt_end", "rt_dose", "num_fraction", "age_at_sct", "age_at_response", "sct_cycles", "age_at_censor_status", "dna_index_numeric"]
...
def get_timing_by_ids(subject_id, timing_id, list):
	ret = [d for d in list if "subject_submitter_id" in d and "timing_id" in d and d["subject_submitter_id"] == subject_id and d["timing_id"] == timing_id]
	...
	for type in types:
		...
		for row in reader:
			...
			if type == "person":
				...
				subject = get_subject_by_id(row["*persons.submitter_id"], "person_id", subjects)
				if subject == None:
					subject = {}
				...
				events = [d for d in timings if "subject_submitter_id" in d and d["subject_submitter_id"] == subject["subject_submitter_id"] ]
				if len(events) > 0:
					tmp_year = None
					for ev in events:
						if "year_at_disease_phase" in ev and ev["year_at_disease_phase"]:
				...
				subject["auth_resource_path"] = "/programs/" + program_name + "/projects/" + project_code + "/persons/" + row["*persons.submitter_id"] + "/subjects/" + row["*submitter_id"]
			elif type == "study":
				subject = get_subject_by_id(row["*subjects.submitter_id"], "subject_submitter_id", subjects)
				if subject == None:
					subject = {}
					ignored_entities.append(subject)
				study = {}
				...
				if subject == None:
					...
					ignored_entities.append(subject)
				molecular_analysis = {}
				if "age_at_molecular_analysis" in row and row["age_at_molecular_analysis"]:
					molecular_analysis["age_at_molecular_analysis"] = toNum(row["age_at_molecular_analysis"])
				...
				if "num_chromosomes" in row and row["num_chromosomes"]:
					molecular_analysis["num_chromosomes"] = toNum(row["num_chromosomes"])
				...
				if "molecular_analysis_classification" in row:
					molecular_analysis["molecular_analysis_classification"] = row["molecular_analysis_classification"]
				...
				if "dna_index_numeric" in row and row["dna_index_numeric"]:
					molecular_analysis["dna_index_numeric"] = toNum(row["dna_index_numeric"])
				...
				if "cytodifferentiation" in row:
					molecular_analysis["cytodifferentiation"] = row["cytodifferentiation"]
				...
				if "molecular_abnormality" in row and row["molecular_abnormality"]:
					molecular_analysis["molecular_abnormality"] = row["molecular_abnormality"]
				if "molecular_abnormality_result" in row and row["molecular_abnormality_result"]:
					molecular_analysis["molecular_abnormality_result"] = row["molecular_abnormality_result"]
				if "translocation_status" in row:
					molecular_analysis["translocation_status"] = row["translocation_status"]
				if "molecular_analysis_method" in row:
					molecular_analysis["molecular_analysis_method"] = row["molecular_analysis_method"]
				if "variant_type" in row:
					molecular_analysis["variant_type"] = row["variant_type"]
				if "molecular_analysis_sample_source" in row:
					molecular_analysis["molecular_analysis_sample_source"] = row["molecular_analysis_sample_source"]

				event = get_timing_by_ids(row["*subjects.submitter_id"], row["timings.submitter_id"], timings)
				if event:
				...
				if row["age_at_tumor_assessment"]:
					tumor_assessment["age_at_tumor_assessment"] = toNum(row["age_at_tumor_assessment"])
				if "longest_diam_dim1" in row and row["longest_diam_dim1"]:
					tumor_assessment["longest_diam_dim1"] = toNum(row["longest_diam_dim1"])
				else:
					tumor_assessment["longest_diam_dim1"] = None
				if "longest_diam_dim2" in row and row["longest_diam_dim2"]:
					tumor_assessment["longest_diam_dim2"] = toNum(row["longest_diam_dim2"])
				if "longest_diam_dim3" in row and row["longest_diam_dim3"]:
					tumor_assessment["longest_diam_dim3"] = toNum(row["longest_diam_dim3"])
				...
				if "parameningeal_extension" in row and row["parameningeal_extension"]:
					tumor_assessment["parameningeal_extension"] = row["parameningeal_extension"]
				if "tumor_classification" in row and row["tumor_classification"]:
					tumor_assessment["tumor_classification"] = row["tumor_classification"]
				if "tumor_detection_method" in row:
					tumor_assessment["tumor_detection_method"] = row["tumor_detection_method"]
				...
				if row["cause_of_death"]:
					survival_characteristic["cause_of_death"] = toArray(row["cause_of_death"])
				...
				if row["age_at_hist_assessment"]:
					histology["age_at_hist_assessment"] = toNum(row["age_at_hist_assessment"])
				if "histology_result_numeric" in row and row["histology_result_numeric"]:
					histology["histology_result_numeric"] = toNum(row["histology_result_numeric"])
				...
				if "somatic_malignancy_type" in row:
					histology["somatic_malignancy_type"] = row["somatic_malignancy_type"]
				...
				if row["age_at_smn"]:
					secondary_malignant_neoplasm["age_at_smn"] = toNum(row["age_at_smn"])
				...
				subject["secondary_malignant_neoplasm"].append(secondary_malignant_neoplasm)
				...
				if row["age_at_procedure"]:
					biopsy_surgical_procedure["age_at_procedure"] = toNum(row["age_at_procedure"])
			
				biopsy_surgical_procedure["margins"] = row["margins"]
				biopsy_surgical_procedure["procedure_site"] = row["procedure_site"]
				biopsy_surgical_procedure["procedure_tumor_classification"] = row["procedure_tumor_classification"]
				...
				if "initial_treatment_category" in row and row["initial_treatment_category"]:
					disease_characteristic["initial_treatment_category"] = row["initial_treatment_category"]
				...
				external_reference["external_resource_icon_path"] = row["external_resource_icon_path"]
				external_reference["external_resource_id"] = row["external_resource_id"]
				external_reference["external_resource_name"] = row["external_resource_name"]
				external_reference["external_subject_id"] = row["external_subject_id"]
				external_reference["external_subject_submitter_id"] = row["external_subject_submitter_id"]
